...
<body>
      ...
      <div class="main-content" id="contas">
        ...
        <div class="container-fluid">
          <div class="row contas">
            ...
            <div id="tabs">
              <ul>
                <li class="tabela-nav"><span>Recebimentos </span></li>
                <li class="tabela-nav"><a href="despesas-fixas.php">Despesas</a></li>
              </ul>
                ...
                <v-toolbar text color="white">
                  <v-dialog v-model="dialog1" max-width="600px">
                    <template v-slot:activator="{ on }">
                      <v-btn v-on="on" color="primary" dark class="mb-2">Adicionar Receita</v-btn>
                    </template>
                    <v-card>
                      <v-card-title>
                      ...
                      <form action="_novaconta.php" method="post" autocomplete="off">
                        <v-card-text>
                          <v-container grid-list-md>
                            <v-layout wrap>
                              <v-flex xs12 sm6 md4>
                                <v-menu ref="datapicker" :close-on-content-click="false" v-model="datapicker" :nudge-right="40" :return-value.sync="date" transition="scale-transition" offset-y>
                                  <v-text-field name='vencimento' required v-model="datanovo" label="Vencimento" slot="activator" readonly="readonly"></v-text-field>
                                  <v-date-picker v-model="datanovo" locale="pt-br" @input="$refs.datapicker.save(date)"></v-date-picker>
                                </v-menu>
                                <v-text-field name='tabela' value='contarecebimento' style='display: none'></v-text-field>
                              </v-flex>
                              <v-flex xs12 sm6 md4>
                              ...
                              <v-flex xs12 sm12 md12>
                                <v-textarea rows="1" id="textarea-novo" auto-grow name="descricao" v-model="editedItem.descricao" label="Descrição" onkeypress="return enterNone(event)"></v-textarea>
                              </v-flex>
                            </v-layout>
                          </v-container>
                        </v-card-text>
                          ...
                          <v-container grid-list-md>
                            <v-layout wrap>
                              <v-flex xs12 sm6 md4 style='display: none;'>
                                ...
                                <v-text-field name='tabela' value='contarecebimento'></v-text-field>
                              </v-flex>
                              <v-flex xs12 sm6 md4>
                                <v-menu ref="datapicker2" :close-on-content-click="false" v-model="datapicker2" :nudge-right="40" :return-value.sync="date" transition="scale-transition" offset-y>
                                  <v-text-field name='vencimento' required v-model="editedItem.vencimentoref" label="Vencimento" slot="activator" readonly="readonly"></v-text-field>
                              ...
                              <v-flex xs12 sm12 md12>
                                <v-textarea rows="1" id="textarea-altera" auto-grow name="descricao" v-model="editedItem.descricaoref" label="Descrição" onkeypress="return enterNone(event)"></v-textarea>
                              </v-flex>
                            </v-layout>
                          </v-container>
                        </v-card-text>
                  ...
                  <v-spacer></v-spacer>
                  <v-text-field v-model="search" append-icon="search" label="Procurar" single-line hide-details></v-text-field>
                </v-toolbar>
                <v-data-table :headers="headers" :items="contarecebimento" :footer-props.items-per-page-options="footerProps" footer-props.items-per-page-text="Linhas por página" no-results-text="Nenhum resultado encontrado" :items="contarecebimento" :search="search" item-key="descricao" :options.sync="options">

                  <template slot="items" slot-scope="props">
                    <td :class='props.item.status'>
                      <v-switch v-model='props.item.pago' color='blue accent-2' @click.native="AlteraStatus(props.item)"></v-switch>
                    </td>
                    <td>{{ props.item.num }}</td>
                    <td>{{ props.item.vencimento }}</td>
                    <td><span v-html="props.item.descricao"></span></td>
                    <td>R$ {{ props.item.valor }}</td>
                    <td><span v-html="props.item.categoria"></span></td>
  ...
  <?php
  $query = "SELECT * FROM contarecebimento";
